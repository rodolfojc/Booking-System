...
public class Controller implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().equals("Register")) {
			...
			//PASSWORD MUST BE MINIMUN 8 CHARATERS, AT LEAST ONE UPPERCASE AND LOWER, SPECIAL CHARACTER AND AT LEAST ONE NUMBER
			...
			//ADDRESS MUST BE AT LEAST WITH 7 CHARACTERS
				...
				//IF THE VALIDATION DOES NOT SET FALSE THE FLAG, THEN THE USER WILL BE REGISTERED
		...
		if(e.getActionCommand().equals("Enter")) {
			...
			//CHECKING IF THE CUSTOMER IS REGISTER IN DATABASE, RETURN TRUE IF THERE IS A MATCH
			Database connCust = new Database(login);
			boolean resultOne = connCust.loginUser("customers", email, password, false);
			
			//CHECKING IF THE PROVIDER IS REGISTER IN DATABASE, RETURN TRUE IF THERE IS A MATCH
			Database connProv = new Database(login);
			boolean resultTwo = connProv.loginUser("providers", email, password, false);
			
			//CHECKING IF THE ADMINISTRATOR IS REGISTER IN DATABASE, RETURN TRUE IF THERE IS A MATCH
